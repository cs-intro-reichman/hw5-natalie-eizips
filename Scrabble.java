...
public class Scrabble {
	...
	public static boolean isWordInDictionary(String word) {
		for (int i = 0; i < DICTIONARY.length; i++){
			if (word.equals(DICTIONARY[i])){ //checks if the given string is equal to string in index, cannot do == in string
				return true;
			}
		}
		return false;
	...
	public static int wordScore(String word) {
		...
		score *= word.length();
		if (word.length() == HAND_SIZE){ // If the length of the word equals the length of the hand, adds 50 points to the score.
			score += 50;
		}
		if (MyString.subsetOf("runi", word)){ // If the word includes the sequence "runi", adds 1000 points to the game.
			score += 1000;
		}
		return score;
	}
	...
	public static void playHand(String hand) {
		...
		while (hand.length() > 0) {
			...
			if (isWordInDictionary(input)){
				score_current += wordScore(input); //current score with word given
				score += score_current; //total score
				hand = MyString.remove(hand, input);
				System.out.println(input + " earned " + score_current + " points. Score: " + score + " points");
				System.out.println();
			}
			else {
